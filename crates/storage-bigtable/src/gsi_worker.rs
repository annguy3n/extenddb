...
use googleapis_tonic_google_bigtable_v2::google::bigtable::v2::{Mutation, mutation};
...
pub async fn run(engine: Arc<BigtableEngine>, cadence: Duration) {
    tracing::info!(
        "bigtable GSI reconciler worker started; cadence={:?}",
        cadence
    );
        ...
        for (k, val) in tables_raw {
            ...
            for gsi in gsis {
                if let Err(e) = reconcile_gsi(engine, &base_key_info, data_table, &gsi).await {
                    tracing::warn!(
                        "reconcile GSI {} for table {} failed: {e}",
                        gsi.index_name,
                        table_name
                    );
                }
...
async fn reconcile_gsi(
    ...
    if let Err(e) = reconcile_gsi_shadow_to_base(engine, base_key_info, data_table, gsi).await {
        tracing::warn!(
            "GSI shadow-to-base reconciliation failed for {}: {}",
            gsi.index_name,
            e
        );
    }
    if let Err(e) = reconcile_gsi_base_to_shadow(engine, base_key_info, data_table, gsi).await {
        tracing::warn!(
            "GSI base-to-shadow reconciliation failed for {}: {}",
            gsi.index_name,
            e
        );
    }
...
async fn reconcile_gsi_shadow_to_base(
    ...
    loop {
        let (items, last) = shadow_qs
            ...
            .await
            .map_err(|e| format!("scan shadow table {shadow_table}: {e}"))?;
        ...
        let base_rows = base_ops
            .batch_get(base_key_info, &base_keys)
            .await
...
async fn reconcile_gsi_base_to_shadow(
    ...
    loop {
        let (items, last) = base_qs
            ...
            .await
            .map_err(|e| format!("scan base table {data_table}: {e}"))?;
        ...
        let targets: Vec<(Item, Item, Vec<u8>)> = items
            .iter()
        ...
        if !targets.is_empty() {
            ...
            let existing_shadows = shadow_ops
                .batch_get(&shadow_key_info, &keys_to_get)
                .await
                .map_err(|e| format!("batch get shadow rows: {e}"))?;

            for ((_base_item, expected_projected, key), existing_shadow) in
                targets.into_iter().zip(existing_shadows)
            {
                match existing_shadow {
                    None => {
                        let mutations = shadow_ops
                            .item_to_mutations(&expected_projected, true)
                            .map_err(|e| format!("item_to_mutations: {e}"))?;
                        shadow_ops
                            .mutate_cells(key, mutations)
                            .await
    ...
    match base_row {
        None => {
            ...
            shadow_ops
                ...
                .await
                .map_err(|e| format!("delete stale shadow row: {e}"))?;
            ...
            if Some(&shadow_key_bytes) == new_shadow_key.as_ref() {
                ...
                if expected_projected != *shadow_item {
                    ...
                    let mutations = shadow_ops
                        ...
                        .map_err(|e| format!("item_to_mutations: {e}"))?;
                    shadow_ops
                        .mutate_cells(shadow_key_bytes, mutations)
                        .await
                        .map_err(|e| format!("repair GSI entry: {e}"))?;
                ...
                shadow_ops
                    ...
                    .await
                    .map_err(|e| format!("delete stale GSI entry: {e}"))?;
                tracing::info!(
                    "GSI Reconciler: deleted stale GSI entry (GSI key changed or sparse)"
                );
